...
var ASK_QUESTION_MAX_CHARS = 8e3;
var ASK_QUOTE_MAX_CHARS = 8e3;
var ASK_ANSWER_MAX_CHARS = 32e3;
function askThreadKey(wait) {
...
function subscribeAskThread(listener) {
...
function isEntry(value) {
  if (value === null || typeof value !== "object") return false;
  const entry = value;
  return typeof entry.id === "string" && typeof entry.question === "string" && (entry.status === "pending" || entry.status === "done" || entry.status === "error");
}
...
function buildHistory(entries, excludeId) {
  return entries.filter((entry) => entry.status === "done" && entry.id !== excludeId).map((entry) => ({
    question: entry.question.slice(0, ASK_QUESTION_MAX_CHARS),
    answer: entry.answer.slice(0, ASK_ANSWER_MAX_CHARS)
  })).slice(-20);
}
function scopeOf(key, cancelledCopy) {
  const existing = scopes.get(key);
  if (existing !== void 0) return existing;
  const stored = readStored(key);
  const entries = (stored ?? []).map((entry) => entry.status === "pending" ? { ...entry, status: "error", error: cancelledCopy } : entry);
  const scope = { key, entries, controller: null };
  scopes.set(key, scope);
  return scope;
}
function commit(scope, entries) {
  if (scopes.get(scope.key) !== scope) return;
  scope.entries = entries;
  persist(scope.key, entries);
  notify();
}
function forgetAskThread(wait) {
...
function startAsk(scope, wait, plan, entry, cancelledCopy, replace) {
  if (scope.controller !== null) return;
  const controller = new AbortController();
  scope.controller = controller;
  commit(scope, replace ? scope.entries.map((item) => item.id === entry.id ? entry : item) : [...scope.entries, entry]);
  ...
  void callAskAi({
    ...
    commit(scope, scope.entries.map((item) => item.id === entry.id ? { ...item, status: "done", answer } : item));
  }).catch((cause) => {
    if (controller.signal.aborted) {
      commit(scope, scope.entries.filter((item) => item.id !== entry.id));
      return;
    }
    const message = cause instanceof AskAiError && cause.code === "cancelled" ? cancelledCopy : cause instanceof Error ? cause.message : String(cause);
    commit(scope, scope.entries.map((item) => item.id === entry.id ? { ...item, status: "error", error: message } : item));
  }).finally(() => {
    if (scope.controller === controller) scope.controller = null;
    notify();
  });
}
function useAskThread(wait, plan, cancelledCopy) {
  const key = askThreadKey(wait);
  const scope = (0, import_react2.useMemo)(() => scopeOf(key, cancelledCopy), [key, cancelledCopy]);
  const entries = (0, import_react2.useSyncExternalStore)(
    subscribeAskThread,
    (0, import_react2.useCallback)(() => scope.entries, [scope])
  );
  const send = (0, import_react2.useCallback)((input) => {
  ...
  const retry = (0, import_react2.useCallback)((entry) => {
    if (scope.controller !== null) return;
    const pending = {
      id: entry.id,
      ...entry.quote !== void 0 ? { quote: entry.quote.slice(0, ASK_QUOTE_MAX_CHARS) } : {},
      question: entry.question.slice(0, ASK_QUESTION_MAX_CHARS),
      answer: "",
      status: "pending"
    };
...
function usePanelMode() {
  const [mode, setMode] = (0, import_react3.useState)(readPanelMode);
...
function PlannotatorReview({
  ...
  const copy = copyOf(t);
  const revision = (0, import_react3.useMemo)(() => planRevision(review.plan), [review.plan]);
  const storageKey = (0, import_react3.useMemo)(() => draftKey(matched), [matched]);
  const restored = (0, import_react3.useMemo)(() => parseStoredDraft(readDraft(storageKey), revision), [storageKey, revision]);
  const [annotations, setAnnotations] = (0, import_react3.useState)(restored?.annotations ?? []);
  const [general, setGeneral] = (0, import_react3.useState)(restored?.general ?? "");
  const [selection, setSelection] = (0, import_react3.useState)(null);
  const [comment, setComment] = (0, import_react3.useState)("");
  const [commentAttention, setCommentAttention] = (0, import_react3.useState)(false);
  const [askDraft, setAskDraft] = (0, import_react3.useState)("");
  const [askQuote, setAskQuote] = (0, import_react3.useState)(null);
  ...
  const [confirmApprove, setConfirmApprove] = (0, import_react3.useState)(false);
  const mode = usePanelMode();
  const previousMode = (0, import_react3.useRef)(mode);
  const [openByMode, setOpenByMode] = (0, import_react3.useState)({
    docked: true,
    drawer: false,
    sheet: false
  });
  const panelOpen = openByMode[mode];
  const panelId = `dsh-plannotator-panel-${(0, import_react3.useId)().replaceAll(":", "")}`;
  const documentRef = (0, import_react3.useRef)(null);
  const commentRef = (0, import_react3.useRef)(null);
  const attentionTimer = (0, import_react3.useRef)(null);
  const launcherRef = (0, import_react3.useRef)(null);
  const panelTitleRef = (0, import_react3.useRef)(null);
  (0, import_react3.useEffect)(() => {
  ...
  const captureSelection = (0, import_react3.useCallback)(() => {
    const root = documentRef.current;
    if (root === null) return;
    ...
    "aside",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "dsh-plannotator-workspace", children: [
          /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("aside", { className: "dsh-plannotator-ask-rail", "aria-label": copy.askTab, children: [
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "dsh-plannotator-ask-rail-title", children: copy.askTab }),
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(
              AskAISection,
              {
          ...
          /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "dsh-plannotator-plan-side", children: [
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(
              "div",
              {
                ref: documentRef,
            ...
            selection !== null && /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)(
              "div",
              {
                ...
                children: [
                  /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("button", { type: "button", onClick: focusComment, children: [
                    "\uFF0B ",
                    copy.commentButton
                  ] }),
            ...
            /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("section", { className: "dsh-plannotator-review", "aria-label": copy.annotation(annotations.length), children: [
              ...
              selection !== null && /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("section", { className: commentAttention ? "dsh-plannotator-new dsh-plannotator-new-attention" : "dsh-plannotator-new", children: [
...
.dsh-plannotator-workspace{display:flex;min-height:0;flex:1;flex-direction:row;background:var(--dsw-alias-bg-base,#fff)}
.dsh-plannotator-ask-rail{display:flex;min-width:0;width:42%;flex:none;flex-direction:column;gap:8px;padding:12px;border-right:1px solid var(--dsw-alias-border-l2,#d9dce3);background:var(--dsw-alias-bg-layer-1,#f7f8fa)}
.dsh-plannotator-ask-rail-title{color:var(--dsw-alias-label-secondary,#596273);font-size:11px;font-weight:650}
.dsh-plannotator-plan-side{display:flex;min-width:0;flex:1;flex-direction:column}
...
.dsh-plannotator-selection-action{position:fixed;z-index:90;display:flex;gap:6px;transform:translateY(8px)}.dsh-plannotator-selection-action>button{padding:6px 10px;border:1px solid var(--dsw-alias-state-business-primary,#4d6bfe);border-radius:9px;background:var(--dsw-alias-button-info-fill,var(--dsw-alias-state-business-primary,#4d6bfe));box-shadow:var(--dsw-shadow-lv2,0 8px 24px rgba(0,0,0,.18));color:var(--dsw-alias-label-primary-foreground,#fff);font:600 12px/18px var(--dsw-font-family,system-ui);cursor:pointer}
 .dsh-plannotator-ask{display:flex;min-height:0;flex:1;flex-direction:column;gap:8px}
 .dsh-plannotator-ask-thread{display:flex;min-height:0;flex:1;flex-direction:column;gap:8px;overflow-y:auto;overscroll-behavior:contain}
 ...
 .dsh-plannotator-ask-composer{display:flex;flex:none;flex-direction:column;gap:7px}
...
.dsh-plannotator-new-attention{box-shadow:0 0 0 3px color-mix(in srgb,var(--dsw-alias-state-business-primary,#4d6bfe) 30%,transparent);transition:box-shadow 160ms ease}
...
@media(min-width:1480px){body:has(>.dsh-plannotator-panel[data-panel-mode='docked'])>#root{width:calc(100% - clamp(560px,40vw,760px))}body:has(>.dsh-plannotator-rail-button[data-panel-mode='docked'])>#root{width:calc(100% - 44px)}.dsh-plannotator-panel[data-panel-mode='docked']{top:0;right:0;bottom:0;width:clamp(560px,40vw,760px);max-width:none;border-width:0 0 0 1px;border-radius:0;box-shadow:none;animation:none}.dsh-plannotator-rail-button[data-panel-mode='docked']{top:0;right:0;bottom:0;width:44px;min-height:0;border-width:0 0 0 1px;border-radius:0;box-shadow:none;transform:none}}
...
@media(prefers-reduced-motion:reduce){.dsh-plannotator-panel{animation:none}.dsh-plannotator-document *{scroll-behavior:auto!important}.dsh-plannotator-new-attention{transition:none}}
`;
